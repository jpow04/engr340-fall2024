...
def main(full_path_to_file):
    ...
    print("Baseline length:", baseline_length)


    # over the baseline, determine the average signal value
    baseline_values = force_plate[0:baseline_length]
    print("Baseline values:", baseline_values)
    baseline = np.mean(baseline_values, axis=0)
    print("Average signal value:", baseline)
    ...
    for index in range(baseline_length, len(force_plate_list)):
        ...
        if value > baseline + delta:
            ...
            print("First landing point index:", first_landing_index)
            print("First landing point:", force_plate_list[first_landing_index])
    ...
    # since we know the take-off point will be afterward.
    for index in range(first_landing_index + 10, len(force_plate_list)):
        ...
        if value < baseline + delta:
            ...
            print("Take off index:", take_off_index)
            print("Take off point:", force_plate_list[take_off_index])
    ...
    for index in range(take_off_index + 10, len(force_plate_list)):
        ...
        if value > baseline + delta:
            ...
            print("Second landing point index:", second_landing_index)
            print("Second landing point:", force_plate_list[second_landing_index])
    ...
    tc = (take_off_index-first_landing_index) / sampling_rate
    print("Tc:", tc)

    # calculate tf and convert to seconds using the sampling rate
    time_of_flight = 0  ### your code here ###
    tf = (second_landing_index-take_off_index) / sampling_rate
    print("Tf:", tf)
...
if __name__ == "__main__":
    ...
    path_to_data_folder = r"C:\Users\powelj\Documents\GitHub\engr340-fall2024\data\drop-jump\force-plate"
    print("Data Folder Path:", path_to_data_folder)  # Print folder path for debug
    ### Do not modify below this line ###
